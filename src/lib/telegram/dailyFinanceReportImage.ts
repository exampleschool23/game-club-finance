...
export function buildDailyFinanceReportSvg(input: DailyFinanceReportInput): string {
  ...
  const clubExpenseRowsSvg = (clubExpenseRows.length > 0
    ? clubExpenseRows
...
<svg xmlns="http://www.w3.org/2000/svg" width="${WIDTH}" height="${HEIGHT}" viewBox="0 0 ${WIDTH} ${HEIGHT}">
  ...
  <g font-family="Noto Sans">
    <rect x="34" y="34" width="1132" height="1532" rx="44" fill="#F8FAF9"/>

    <rect x="68" y="68" width="1064" height="594" rx="34" fill="#FFFFFF" stroke="#C9D8D5" stroke-width="2"/>
    ...
    <rect x="68" y="686" width="1064" height="278" rx="34" fill="#FFFFFF" stroke="#C9D8D5" stroke-width="2"/>
    <text x="108" y="746" font-size="34" font-weight="850" fill="${BLUE}">ДОХОД БАРА — ${escapeXml(money(input.barSales))}</text>
    <line x1="108" y1="774" x2="1092" y2="774" stroke="#E7EEEC" stroke-width="2"/>
    <text x="108" y="830" font-size="32" font-weight="850" fill="${RED}">РАСХОДЫ БАРА — ${escapeXml(money(input.barExpenses))}</text>
    <line x1="108" y1="858" x2="1092" y2="858" stroke="#E7EEEC" stroke-width="2"/>
    <text x="108" y="915" font-size="32" font-weight="850" fill="${RED}">ЗАКУПКИ СКЛАДА — ${escapeXml(money(input.stockPurchases))}</text>

    <rect x="68" y="988" width="1064" height="86" rx="28" fill="#FFFFFF" stroke="#DDE8E5" stroke-width="2"/>
    <text x="108" y="1044" font-size="31" font-weight="850" fill="${RED}">ОБЩИЕ РАСХОДЫ — ${escapeXml(money(input.totalExpenses))}</text>

    ${summaryCard(68, 1098, 'ОСТАТОК ДЕНЕГ КЛУБА ЗА МЕСЯЦ', input.gameClubMoneyLeft, '#15803D')}
    ${summaryCard(615, 1098, 'СРЕДНИЙ ДНЕВНОЙ ДОХОД КЛУБА', input.averageDailyGameClubIncome, PURPLE)}
    ${summaryCard(68, 1288, 'ДОХОД БАРА ЗА МЕСЯЦ', input.barMoneyLeft, ORANGE)}
    ${summaryCard(615, 1288, 'СТОИМОСТЬ СКЛАДА', input.inventoryValue, INK)}

    <rect x="68" y="1478" width="1064" height="68" rx="24" fill="${RED}"/>
    <text x="108" y="1523" font-size="30" font-weight="850" fill="#FFFFFF">АКТИВНЫЕ ДОЛГИ</text>
    <text x="1090" y="1523" text-anchor="end" font-size="32" font-weight="850" fill="#FFFFFF">${escapeXml(money(input.activeDebts))}</text>
  </g>
</svg>`;
}
