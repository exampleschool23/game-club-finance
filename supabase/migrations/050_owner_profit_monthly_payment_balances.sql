-- Add monthly payment-method balances; bar money remains in monthlyBalances.
-- Bar sales have no payment method and must not be assigned to a cash bucket.
...
begin
  ...
  return jsonb_build_object(
    ...
    'paymentMethodBalancesByMonth', coalesce((
      with amounts as (
        select date_trunc('month', entries.date)::date as period_month,
          coalesce(entries.cash_income, 0) as cash,
          coalesce(entries.terminal_income, 0) as terminal,
          coalesce(entries.card_income, 0) as card,
          coalesce(entries.playstation_income, 0) as playstation
        from public.daily_cash_entries entries
        where entries.club_id = p_club_id and entries.date <= p_through_date
        union all
        select date_trunc('month', payments.date)::date,
          case when payments.payment_method = 'cash' then payments.amount else 0 end,
          case when payments.payment_method = 'terminal' then payments.amount else 0 end,
          case when coalesce(payments.payment_method, 'card') not in ('cash', 'terminal') then payments.amount else 0 end,
          0
        from public.debt_payments payments
        where payments.club_id = p_club_id and payments.date <= p_through_date
        union all
        select date_trunc('month', expenses.date)::date,
          case when expenses.payment_method = 'cash' then -expenses.amount else 0 end,
          case when expenses.payment_method = 'terminal' then -expenses.amount else 0 end,
          case when coalesce(expenses.payment_method, 'card') not in ('cash', 'terminal') then -expenses.amount else 0 end,
          0
        from public.expenses expenses
        where expenses.club_id = p_club_id and expenses.date <= p_through_date
          and coalesce(expenses.payment_source, 'game_club') = 'game_club'
      ), monthly as (
        select to_char(period_month, 'YYYY-MM') as month,
          jsonb_build_object('cash', sum(cash), 'terminal', sum(terminal),
            'card', sum(card), 'playstation', sum(playstation)) as balances
        from amounts group by period_month
      )
      select jsonb_object_agg(month, balances) from monthly
    ), '{}'::jsonb),
