-- Carry receipts across dates with no saved closing. No existing ledger rows
-- are rewritten by this migration. Historical corrections remain explicit
-- owner saves through the atomic RPC, including its forward-chain validation.
create index if not exists idx_stock_purchases_club_product_date
  on public.stock_purchases (club_id, product_id, date);

-- Keep get_latest_stock_closings unchanged for older clients. This read returns
-- opening balances, not raw closings; RLS applies to both source ledgers.
create or replace function public.get_stock_opening_balances(
  p_club_id uuid,
  p_before_date date
)
returns table (product_id uuid, previous_stock numeric)
language sql
stable
security invoker
...
as $$
  with latest as (
    select distinct on (counts.product_id)
      counts.product_id, counts.date, counts.closing_stock
    from public.daily_stock_counts counts
    where counts.club_id = p_club_id and counts.date < p_before_date
    order by counts.product_id, counts.date desc, counts.id
  ), receipts as (
    select purchases.product_id, sum(purchases.quantity) as quantity
    from public.stock_purchases purchases
    left join latest on latest.product_id = purchases.product_id
    where purchases.club_id = p_club_id
      and purchases.date < p_before_date
      and (latest.date is null or purchases.date > latest.date)
      and (latest.date is not null or p_before_date < public.club_business_date(p_club_id))
    group by purchases.product_id
  )
  select coalesce(latest.product_id, receipts.product_id),
    coalesce(latest.closing_stock, 0) + coalesce(receipts.quantity, 0)
  from latest full join receipts on receipts.product_id = latest.product_id;
$$;

revoke all on function public.get_stock_opening_balances(uuid, date) from public, anon;
grant execute on function public.get_stock_opening_balances(uuid, date) to authenticated;
...
begin
  ...
  loop
    ...
    select counts.closing_stock, counts.date
    ...
    -- Same-day purchases stay in added_today. Receipts on skipped dates
    -- belong to the opening balance and must not disappear between closings.
    select coalesce(sum(purchases.quantity), 0)
    into v_carried_purchases
    from public.stock_purchases purchases
    where purchases.club_id = p_club_id
      and purchases.product_id = v_product_id
      and (v_previous_date is null or purchases.date > v_previous_date)
      and purchases.date < p_date;

    if v_previous is not null then
      v_previous := v_previous + v_carried_purchases;
    end if;
    ...
    if not v_product.tracks_inventory then
      ...
      if v_previous is null then
        v_previous := case
          ...
          else v_carried_purchases
        end;
      end if;

      if v_has_existing and p_date < v_business_date then
        -- Keep an existing historical row editable using its displayed
        -- snapshot. An explicit edit to an earlier closing still recalculates
        -- the forward chain below; deployment alone must not alter history.
        v_previous := v_existing.previous_stock;
  ...
  with closing_timeline as (
    ...
    select closing_timeline.*,
      previous_closing + coalesce((
        select sum(purchases.quantity)
        from public.stock_purchases purchases
        where purchases.club_id = p_club_id
          and purchases.product_id = closing_timeline.product_id
          and purchases.date > closing_timeline.previous_date
          and purchases.date < closing_timeline.date
      ), 0) as canonical_previous
    from closing_timeline
  )
