-- Return compact monthly owner-profit aggregates instead of transferring every
-- underlying ledger row to the browser.
    ...
    'monthlyBalances', coalesce((
      with monthly_values as (
        select date_trunc('month', entries.date)::date as period_month,
          sum(coalesce(entries.cash_income, 0)
            + coalesce(entries.terminal_income, 0)
            + coalesce(entries.card_income, 0)
            + coalesce(entries.playstation_income, 0)) as game_club_earned,
          0::numeric as bar_earned,
          0::numeric as game_club_withdrawn,
          0::numeric as bar_withdrawn
        ...
        select date_trunc('month', payments.date)::date,
          sum(payments.amount), 0::numeric, 0::numeric, 0::numeric
        from public.debt_payments payments
        where payments.club_id = p_club_id and payments.date <= p_through_date
        group by 1

        union all
        select date_trunc('month', counts.date)::date,
          0::numeric, sum(counts.bar_income), 0::numeric, 0::numeric
        from public.daily_stock_counts counts
        where counts.club_id = p_club_id and counts.date <= p_through_date
        group by 1

        union all
        select date_trunc('month', purchases.date)::date,
          0::numeric, -sum(purchases.quantity * purchases.cost_price), 0::numeric, 0::numeric
        from public.stock_purchases purchases
        where purchases.club_id = p_club_id
          and purchases.date between date '2026-07-02' and p_through_date
        group by 1

        union all
        select date_trunc('month', expenses.date)::date,
          -sum(expenses.amount) filter (where expenses.payment_source = 'game_club'),
          -sum(expenses.amount) filter (where expenses.payment_source = 'bar'),
          0::numeric, 0::numeric
        ...
        union all
        select withdrawals.period_month,
          0::numeric, 0::numeric,
        ...
        select period_month,
          coalesce(sum(game_club_earned), 0) as game_club_earned,
      ...
      select jsonb_agg(to_jsonb(balances) order by period_month)
      from balances
    ), '[]'::jsonb),
    'withdrawalRows', coalesce((
      ...
      from (
        ...
        where club_id = p_club_id
          and period_month <= date_trunc('month', p_through_date)::date
      ) rows
    ), '[]'::jsonb)
  );
end;
